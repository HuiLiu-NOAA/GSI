...
!   2020-08-11  hliu  - Modifications to use linear bias correction of Aeolus winds
...
! hliu  real(r_kind) zob,termrg,dz,termr,sin2,termg
  real(r_kind) zob,termrg,dz,termr,sin2,termg, zobt, zobb,zobt0, zobb0  
...
!hliu ---------- for Aeolus linear bias correction ----------
  integer(i_kind), parameter:: nlat=19, nray= 21, nmie=17
  real(r_kind), Parameter :: pi = 3.1415926, d2r= pi/180.0

real(r_single) rayasc(2,nray,nlat), raydes(2,nray,nlat), biasray(2,nray,nlat)
real(r_single) mieasc(2,nmie,nlat), miedes(2,nmie,nlat), biasmie(2,nmie,nlat)
real(r_single) hght_ray(nray), hght_mie(nmie), latint(nlat)

real(r_single) dwwindt, dwwindb, l2bobs, lat0, obsdif, latdif, bc6
integer(i_kind) n, lnd, lnd2, ireg, kk
real(r_single)  x1, x2, f1, f2, xx

real(r_single)  rayerr(3,nray), mieerr(3,nmie)

 data hght_ray/22.5, 20.5, 18.5, 17.5, 16.5, 15.5, 14.5, 13.5, 12.5, 11.5, &
            10.5,  9.5,  8.5,  7.5,  6.5,  5.5,  4.5,  3.5,  2.5,  1.5,0.5/

 data hght_mie /16.5, 15.5, 14.5, 13.5, 12.5, 11.5, 10.5, 9.5,  8.5, 7.5,  &
...
! hliu
   ...
   if( jiter == 1) then
      ...
      call read_L2B_bias_correction2019linear
!hliu -----------------------------------
...
!  if( dist_km < 90.0  .and. dist_hh < 250.0  .and. muse(k) .and. muse(l) &
! hliu 2020.06.21
...
!hliu-------------------------------------------------------------
! Use error estimate of Aeolus winds for NH, SH, and TR region

 if (ictype(ikx)==48 ) then           ! for Aeolus winds
...
!hliu------------------------------------------------------------------------
! Apply the linear Bias corrections in bins of 10 deg lat and height levels
!----------------------------------------------------------------------------
...
! --- interpolate the bias correction to Aeolus location ----
  if( icsubtype(ikx)==20) then           ! Rayleigh clear-sky

       kk= minloc( abs(hght_ray - zob), 1)
!-----------------------------------------------------
       if( lnd .ne. lnd2) then
           f1 = biasray(1, kk, lnd)  + biasray(2, kk, lnd) * l2bobs
           f2 = biasray(1, kk, lnd2) + biasray(2, kk, lnd2)* l2bobs
         bc6 = f1 + (xx-x1)/(x2-x1) * (f2-f1)
        else
           f1 = biasray(1, kk, lnd)  + biasray(2, kk, lnd) * l2bobs
         bc6 = f1
       endif
   print*, 'bc6ray= ', bc6, zob, kk, lat0, lnd, lnd2

   else        ! Mie cloudy

       kk= minloc( abs(hght_mie - zob), 1)
       if( lnd .ne. lnd2) then
           f1 = biasmie(1, kk, lnd)  + biasmie(2, kk, lnd) * l2bobs
           f2 = biasmie(1, kk, lnd2) + biasmie(2, kk, lnd2)* l2bobs
         bc6 = f1 + (xx-x1)/(x2-x1) * (f2-f1)
        else
           f1 = biasmie(1, kk, lnd)  + biasmie(2, kk, lnd) * l2bobs
         bc6 = f1
       endif
   print*, 'bc6mie= ', bc6, zob, kk, lat0, lnd, lnd2
   endif

!---------------------------------------------------
        ddiff = ddiff - bc6       ! apply bias correction
 endif
!hliu -------------------------------
...
! QC Rayleigh clear:
   if(icsubtype(ikx)==20 ) then
       if( ianldate < 2019080200 ) then   ! skip rayleigh wind before Aug 2 00Z 2019
        muse(i)=.false.
        ratio_errors = zero
        data(iuse,i) = 200
       endif

       if( data(ihgt,i) <= 2000.0 .or. data(ihgt,i) > 26000.0) then
        muse(i)=.false.
        ratio_errors = zero
        data(iuse,i) = 200
       endif
...
!!!hliu------------------------------------------------------------
!!!  Apply blacklist of L2B winds likely affected by "hot pixels".
!!!  Rayleigh winds around (rang bin=11, 6.2km), 12.3km, 19.8km.
!!!  not needed for reprocessed Aeolus winds
!!!hliu------------------------------------------------------------
!!     if(icsubtype(ikx)==20) then
!!       if(abs(data(ihgt,i)- 6200.0_r_kind)<500.0_r_kind .or. &
!!          abs(data(ihgt,i)-12300.0_r_kind)<500.0_r_kind .or. &
!!          abs(data(ihgt,i)-19800.0_r_kind)<500.0_r_kind) then
!!           muse(i)=.false.
!!     ratio_errors = zero
!!           data(iuse,i) = 202
!!       endif
...
!!       if(data(ihgt,i)>13000.0_r_kind) then
!!           muse(i)=.false.
!!     ratio_errors = zero
!!           data(iuse,i) = 202
!!       endif
...
!hliu -------------------------------------
subroutine read_L2B_bias_correction2019linear

     rayasc = 0.0; raydes = 0.0;  mieasc = 0.0; miedes = 0.0

   open(961, file='Bias_correction_ray_coef0.asc',form='formatted')
   open(962, file='Bias_correction_ray_coef1.asc',form='formatted')
   open(963, file='Bias_correction_ray_coef0.des',form='formatted')
   open(964, file='Bias_correction_ray_coef1.des',form='formatted')

   open(965, file='Bias_correction_mie_coef0.asc',form='formatted')
   open(966, file='Bias_correction_mie_coef1.asc',form='formatted')
   open(967, file='Bias_correction_mie_coef0.des',form='formatted')
   open(968, file='Bias_correction_mie_coef1.des',form='formatted')


    do k=1, nray
     read(961, '(19f8.3)') (rayasc(1,k,n), n=1, nlat)
     read(962, '(19f8.3)') (rayasc(2,k,n), n=1, nlat)

     read(963, '(19f8.3)') (raydes(1,k,n), n=1, nlat)
     read(964, '(19f8.3)') (raydes(2,k,n), n=1, nlat)
    enddo

    do k=1, nmie
     read(965, '(19f8.3)') (mieasc(1,k,n), n=1, nlat)
     read(966, '(19f8.3)') (mieasc(2,k,n), n=1, nlat)

     read(967, '(19f8.3)') (miedes(1,k,n), n=1, nlat)
     read(968, '(19f8.3)') (miedes(2,k,n), n=1, nlat)
    enddo

    close(961);close(962);close(963);close(964)
    close(965);close(966);close(967);close(968)
...
!----------- rad in HL error estimates -------------
   ...
   open(983, file='Mie_errTR.txt',form='formatted')
    ...
    do k=1, nray
     read(971, '(2f8.2)')  rayerr(1,k)    ! NH
     read(972, '(2f8.2)')  rayerr(2,k)    ! SH
     read(973, '(2f8.2)')  rayerr(3,k)    ! TR
    end do

    do k=1, nmie
     read(981, '(2f8.2)')  mieerr(1,k)    ! NH
     read(982, '(2f8.2)')  mieerr(2,k)    ! SH
     read(983, '(2f8.2)')  mieerr(3,k)    ! TR
    end do
...
print*, 'mieerr= ', mieerr
 ...
 end subroutine read_L2B_bias_correction2019linear
